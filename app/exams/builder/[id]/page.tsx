...
try {
  ...
  // 3. إرسال إشعارات داخلية + Push عند نشر الاختبار
    ...
    try {
      ...
      if (students && students.length > 0) {
        ...
        await fetch('/api/push/send', {
          ...
          body: JSON.stringify({
            userIds: studentIds,
            title: '📋 اختبار جديد — مدرسة الرفعة',
            body: `اختبار جديد في مادة ${subjectName}: ${exam.title}`,
            url: `/exams/take/${examId}`,
          }),
        });
      }
    } catch (notifErr) {
...
className={`fixed top-6 left-1/2 z-50 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 border backdrop-blur-md ${ notification.type === 'success' ? 'bg-emerald-50/90 text-emerald-800 border-emerald-200' : 'bg-red-50/90 text-red-800 border-red-200' }`}
>
