        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl bg-white/80 backdrop-blur-xl p-6 shadow-sm ring-1 ring-slate-200/50">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Link href="/assignments" className="text-xs font-bold text-amber-600 hover:text-amber-700 bg-amber-50 px-2 py-1 rounded-lg">الكل</Link>
            </div>
            <div className="space-y-3">
              {upcomingAssignments.length > 0 ? upcomingAssignments.map(a => (
                <Link key={a.id} href={`/assignments/${a.id}`} className="block group">
                  <div className="p-3 rounded-2xl bg-white border border-slate-100 hover:border-amber-200 hover:shadow-sm transition-all">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] font-black text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full uppercase tracking-wider">{a.subject?.name}</span>
                      <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                        <Clock className="h-3 w-3" />
                        {format(new Date(a.due_date), "d MMM", { locale: arSA })}
                      </span>
                    </div>
                    <p className="font-bold text-slate-900 text-sm group-hover:text-amber-600 transition-colors truncate">{a.title}</p>
                  </div>
                </Link>
              )) : (
                <div className="text-center py-6 text-slate-400 bg-slate-50 rounded-2xl border border-dashed border-slate-200 font-bold text-xs">لا توجد واجبات حالياً</div>
              )}
            </div>
          </div>
        </div>
