...
export default function SchedulePage() {
  ...
  // تحديث الوقت كل دقيقة لتمييز الحصة الحالية
  useEffect(() => {
                ...
                DAYS.map((day) => (
                  <React.Fragment key={day.id}>
                    ...
                    {periods.map(p => {
                      ...
                      const periodStatus = getPeriodStatus(day.id, period);
                      return (
                        <div key={`${day.id}-${period}`} className={`group p-3 border rounded-xl min-h-[110px] flex flex-col items-center justify-center text-center transition-all relative overflow-hidden
                          ${slot && periodStatus === 'current'
                            ? 'bg-gradient-to-br from-emerald-500 to-emerald-600 text-white shadow-lg shadow-emerald-200 border-emerald-400 scale-[1.04] z-20'
                            : slot && periodStatus === 'next'
                            ? 'bg-gradient-to-br from-amber-400 to-amber-500 text-white shadow-lg shadow-amber-200 border-amber-300 scale-[1.02] z-10'
                            : slot && periodStatus === 'past'
                        ...
                        >
                          {slot && periodStatus === 'current' && (
                            <div className="absolute inset-0 rounded-xl pointer-events-none">
                              ...
                              <div className="absolute -top-1.5 -right-1.5 bg-emerald-700 text-white text-[8px] font-black px-1.5 py-0.5 rounded-full border border-white z-30">
                                ⚡ الآن
                              </div>
                            </div>
                          )}
                          {slot && periodStatus === 'next' && (
                            <div className="absolute -top-1.5 -right-1.5 bg-amber-700 text-white text-[8px] font-black px-1.5 py-0.5 rounded-full border border-white z-30">
                              استعد!
                            </div>
                          )}
                          {displaySlot ? (
